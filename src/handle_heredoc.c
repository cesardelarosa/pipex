...
/*   Updated: 2025/03/08 12:55:54 by cesi             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	create_temp_file(char **temp_file, t_context *ctx)
{
	*temp_file = generate_temp_name();
	if (!*temp_file)
	{
		error_exit("malloc failed", NULL, ctx);
		return (0);
	}
	return (1);
}
...
static void	cleanup_temp_file(int fd, char *temp_file, t_context *ctx)
{
	...
	{
		free(temp_file);
		error_exit(strerror(errno), "heredoc", ctx);
	}
	unlink(temp_file);
	free(temp_file);
	register_fd(fd, ctx);
	...
	{
		close(fd);
		error_exit(strerror(errno), "heredoc dup2", ctx);
	}
}
...
int	handle_heredoc(t_redir *redir, t_context *ctx)
{
	...
	if (!read_input_to_file(fd, redir->file))
	{
		close(fd);
		free(temp_file);
		error_exit("warning: heredoc delimited by end-of-file",
			redir->file, ctx);
	}
